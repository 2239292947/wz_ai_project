    ...
    <div className="space-y-4 fade-in">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-text">选择解析规则</h2>
          <div className="flex gap-2">
            <button
              onClick={handleAiGenerate}
              disabled={isAiGenerating}
              className="px-4 py-1.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 transition-colors"
            >
              {isAiGenerating ? (
                <>
            ...
            <button
              ...
              className="px-4 py-1.5 border border-primary text-primary rounded-lg text-sm font-medium hover:bg-primary-light transition-colors"
            >
              + 新建规则
            </button>
          </div>
        ...
        {Object.keys(aiConfidence).length > 0 && showNewRule && (
          <div className="mb-4 p-3 bg-warning-bg border border-orange-200 rounded-lg text-sm">
            <p className="font-medium text-warning mb-1">AI推测标注：</p>
            {Object.entries(aiConfidence).map(([field, msg]) => (
              <p key={field} className="text-text-secondary">- {field}: {msg}</p>
            ))}
            <p className="mt-1 text-text-muted">请确认以下规则配置是否正确，可手动调整后保存</p>
          </div>
        )}

        {/* New Rule Form */}
        {showNewRule && (
          <div className="mb-4 p-4 bg-primary-light/50 border border-teal-200 rounded-xl space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-medium text-text">新建解析规则</h3>
              <button
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">规则配置 (JSON)</label>
              <textarea
                value={newRuleConfig}
                onChange={e => setNewRuleConfig(e.target.value)}
                rows={12}
                className="w-full px-3 py-2 border border-border rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-1.5 border border-primary text-primary rounded-lg text-sm font-medium hover:bg-primary-light disabled:opacity-50 transition-colors"
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-4 h-4 rounded-full border-2 ${
                      selectedRuleId === rule.id ? 'border-primary bg-primary' : 'border-gray-300'
                    }`}>
                      {selectedRuleId === rule.id && (
                  ...
                  <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                    <button
                      ...
                      className="px-2 py-1 text-xs text-primary hover:bg-primary-light rounded transition-colors"
                      title="试解析"
                    >
                      试解析
                    </button>
                    <button
                      ...
                      className="px-2 py-1 text-xs text-text-muted hover:bg-gray-100 rounded transition-colors"
                      title="复制"
                    >
                      复制
                    </button>
                    {editingRuleId === rule.id ? (
                      <>
                        <button
                          ...
                          className="px-2 py-1 text-xs text-success hover:bg-success-bg rounded transition-colors"
                        >
                          保存
                        </button>
                        <button
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="px-2 py-1 text-xs text-error hover:bg-error-bg rounded transition-colors"
                          title="删除"
                        >
                          删除
                        </button>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-6 py-2.5 bg-primary text-white rounded-lg font-medium hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
        >
          {isParsing ? (
            <>
              <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
      ...
      {isParsing && (
        <div className="mt-4">
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div
              className="h-full bg-primary rounded-full transition-all duration-300 progress-pulse"

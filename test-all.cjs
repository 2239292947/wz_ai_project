const { readFileContent } = require('./src/lib/file-reader');
const { executeParse, ordersToFlatRows } = require('./src/lib/parser-engine');
const fs = require('fs');
...
async function testParse(filePath, ruleConfig, label) {
  ...
  try {
    ...
    if (flatRows.length > 0) {
      console.log('Sample:', JSON.stringify(flatRows[0]));
      if (flatRows.length > 1) console.log('Sample2:', JSON.stringify(flatRows[1]));
    }
  } catch(e) {
...
(async () => {
  // 1. 黎明屯
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/12.25海口龙湖天街-配送发货单PS2512220005001(1).xlsx', {
    fileType:'excel',sheets:'all',
  ...
  // 2. 湖南仓
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/湖南仓.xlsx', {
    fileType:'excel',sheets:'all',
  ...
  // 3. 多门店分Sheet
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/多门店分Sheet出库单.xlsx', {
    fileType:'excel',sheets:'all',
    dataRegion:{mode:'tabular',headerRow:3,dataStartRow:4,dataEndMarker:'合计'},
    fieldMappings:[
      {targetField:'skuCode',source:'column_header',value:'物品编码'},
  ...
  // 4. 卡片式
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/门店调拨单-卡片式.xlsx', {
    fileType:'excel',sheets:'all',
  ...
  // 5. 欢乐牧场
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/欢乐牧场模板0430.xlsx', {
    fileType:'excel',sheets:'all',
    ...
    transformations:[{type:'matrix_transpose',config:{
      fixedColumns:[{sourceLabel:'SKU名称',targetField:'skuName'},{sourceLabel:'外部商品编码',targetField:'skuCode'},{sourceLabel:'规格',targetField:'spec'}],
      transposeStartCol:13,transposeEndCol:18,
      transposeHeaderTarget:'storeName',transposeValueTarget:'quantity',
      skipIfZero:true,skipIfEmpty:true
    }}]
  }, '欢乐牧场');

  // 6. PDF - 黔寨寨
  await testParse('C:/Users/Administrator/Desktop/AIK/demos/黔寨寨贵州烙锅（鞍山店）常温.pdf', {
    fileType:'pdf',sheets:'all',
